...
def test_kitti_2012(cfg, model, gt_flows, noc_masks, writer=None, iter=None):
    ...
    eval_flow_res = eval_flow_avg(gt_flows, noc_masks, flow_list, cfg, write_img=False, writer=writer, iter=iter)
...
def test_kitti_2015(cfg, model, gt_flows, noc_masks, gt_masks, writer=None, depth_save_dir=None, iter=None):
    ...
    #pdb.set_trace()
    eval_flow_res = eval_flow_avg(gt_flows, noc_masks, pred_flow_list, cfg, moving_masks=gt_masks, write_img=False, writer=writer, iter=iter)
    print('CONFIG: {0}, mode: {1}'.format(cfg.config_file, cfg.mode))
