...
def train(cfg):
    ...
    for iter_, inputs in enumerate(tqdm(dataloader)):
        if (iter_ + 1) % cfg.test_interval == 0 and (not cfg.no_test):
            ...
            if cfg.dataset == 'kitti_depth' or cfg.dataset == 'kitti_odo' or cfg.dataset == 'sintel_raw':
                if not (cfg.mode == 'depth' or cfg.mode == 'flowposenet'):
                    eval_2012_res = test_kitti_2012(cfg, model_eval, gt_flows_2012, noc_masks_2012, writer, iter=iter_)
                    eval_2015_res = test_kitti_2015(cfg, model_eval, gt_flows_2015, noc_masks_2015, gt_masks_2015, writer, iter=iter_, depth_save_dir=os.path.join(cfg.model_dir, 'results'))
        ...
        if iter_ % cfg.log_interval == 0:
            visualizer.print_loss(loss_pack, writer, iter_=iter_)
